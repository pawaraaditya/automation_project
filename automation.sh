...
if which apache2 > /dev/null ; then
	echo "already installed here : $(dpkg-query -W -f='${Version}\n' apache2)"
else
   echo "Installing now"
...
then
    echo "apache2 is not running !! restarting now"
...
aws s3 	cp /tmp/$myname-$logtype-${timestamp}.$type s3://$s3_aaditya/$myname-$logtype-${timestamp}.$type
